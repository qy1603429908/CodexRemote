...
export type ServerMessagePayload =
  ...
  | { type: "approval"; approval: WireApprovalRequest }
  | { type: "approvals.snapshot"; approvals: WireApprovalRequest[] }
...
export type ThreadState = 'unknown' | 'not_loaded' | 'idle' | 'running' | 'waiting_approval' | 'waiting_input' | 'error';
export type CompactionPhase = 'requested' | 'running' | 'retrying' | 'succeeded' | 'failed';
...
export interface RemoteThread {
  ...
  tokenUsage?: { totalTokens: number; inputTokens: number; outputTokens: number; reasoningOutputTokens: number };
}
